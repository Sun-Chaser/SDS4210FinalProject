...
```
```{r}
plot_missing_hist <- function(df, binwidth = 0.05) {
  library(ggplot2)
  library(scales)
  ...
  ggplot(df, aes(x = pct_missing)) +
    ...
    labs(
      title = "Distribution of Missingness by Firm",
      x = "Percent of variables missing",
      y = "Number of firms"
    ) +
    theme_minimal()
}

plot_missing_hist(clean_data_result$summary)
```

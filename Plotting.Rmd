...
# 1. Identify quarter columns (e.g., "2008Q1", "2009Q2", ...)
quarter_cols <- names(cleaned_data)[grepl("^\\d{4}Q[1-4]$", names(cleaned_data))]

# 2. Pivot to long and count submissions
submissions_per_quarter <- cleaned_data %>%
  pivot_longer(
    cols = all_of(quarter_cols),
    names_to = "quarter",
    values_to = "value"
  ) %>%
  filter(!is.na(value)) %>%          # only keep actual submissions
  count(quarter, name = "n_submissions") %>%
  mutate(
    quarter = factor(quarter, levels = sort(unique(quarter)))  # keep chronological order
  )

# 3. Plot histogram / bar plot of number of submissions per quarter
ggplot(submissions_per_quarter, aes(x = quarter, y = n_submissions)) +
  geom_col() +
  labs(
    title = "Number of Submissions per Quarter",
    x = "Quarter",
